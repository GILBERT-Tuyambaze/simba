...
from core.store_roles import can_access_dashboard, can_update_existing_roles, normalize_store_role
...
async def _get_actor_branch(service: User_profilesService, user_id: str) -> Optional[str]:
    profile = await service.get_by_field("user_id", user_id)
...
async def query_user_profiless_all(
    ...
    try:
        ...
        if not can_access_dashboard(actor_role):
            raise HTTPException(status_code=403, detail="Dashboard profile access is not available for customers")
        ...
        if actor_role != "super_admin":
            actor_branch = await _get_actor_branch(service, str(current_user.id))
            result["items"] = [
                item for item in result["items"] if not actor_branch or item.default_branch == actor_branch
            ]
...
@router.get("/directory", response_model=User_profilesListResponse)
async def query_user_profiles_directory(
    ...
    service = User_profilesService(db)
    actor_role = normalize_store_role(current_user.role)
    query_dict = {"default_branch": branch}
    if role:
        query_dict["role"] = normalize_store_role(role)

    result = await service.get_list(skip=0, limit=200, query_dict=query_dict, sort="-id")
    if actor_role == "customer":
        result["items"] = [
            item
            for item in result["items"]
            if normalize_store_role(item.role) in {"delivery_agent", "branch_staff", "branch_manager"}
        ]
    return result
...
async def create_user_profiles(
    ...
    try:
        ...
        if not can_update_existing_roles(current_user.role):
            payload["role"] = normalize_store_role(current_user.role)
...
async def update_user_profiless_batch(
    ...
    try:
        for item in request.items:
            ...
            if "role" in update_dict and not can_update_existing_roles(current_user.role):
                update_dict["role"] = normalize_store_role(current_user.role)

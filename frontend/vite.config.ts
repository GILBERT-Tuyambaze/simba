...
process.env.VITE_APP_TITLE ??= process.env.OVERVIEW_TITLE ?? 'Simba Supermarket';
process.env.VITE_APP_DESCRIPTION ??= process.env.OVERVIEW_DESCRIPTION ?? 'Simba Supermarket - groceries, drinks, home essentials, and delivery in Kigali.';
...
export default defineConfig(({ command }) => {
  ...
  return {
    plugins: [
      ...
      atoms(),
      Sitemap({
        hostname: process.env.VITE_SITE_URL?.trim() || 'http://127.0.0.1:3000',

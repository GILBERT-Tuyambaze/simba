...
function getLocalizedAuthError(t: (key: string, fallback?: string) => string, error: unknown): string {
  ...
  if (
    lowered.includes('supabase is not configured') ||
    lowered.includes('supabase auth')
  ) {
    return 'Set Supabase environment variables to enable login.';
  }
            ...
            <section className="space-y-6">
              ...
              <div className="max-w-xl space-y-3 border border-border/60 bg-card/82 p-4 text-sm text-muted-foreground backdrop-blur-sm">
                <div className="flex items-start gap-3">
                  ...
                  <p>
                    Supabase manages identity. Your profile, orders, and admin role are protected by database RLS.
                  </p>
                </div>
                <div className="flex items-start gap-3">
              ...
              <CardContent className="space-y-5 pt-6">
                ...
                <form onSubmit={handleEmailSubmit} className="space-y-4">
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    <Button type="submit" className="w-full" disabled={busy || supabaseMissing}>
                      {busy ? (
                        <span className="inline-flex items-center gap-2">
                          <LoaderCircle className="h-4 w-4 animate-spin" />
                          {loginPhase === 'session' ? t('auth.generatingSession') : t('auth.matchingCredentials')}

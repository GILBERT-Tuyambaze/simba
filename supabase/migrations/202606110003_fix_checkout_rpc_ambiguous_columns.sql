-- Fix runtime ambiguity in place_order_with_inventory.
-- The RETURNS TABLE column `discount` conflicts with unqualified product
-- column references inside PL/pgSQL. Qualify product columns explicitly.
...
begin
  ...
  loop
    ...
    select p.id, p.name, p.price, p.discount, p.image, p.unit, p.discontinued
    into v_product
  ...
  for v_item in
    ...
    order by (item->>'product_id')::bigint
  loop
    select p.id, p.name, p.price, p.discount, p.image, p.unit
    into v_product
    from public.products p
    where p.id = v_item.product_id;
    ...
    set stock_count = greatest(pi.stock_count - v_item.quantity, 0),
        updated_at = v_now
    where pi.product_id = v_item.product_id
      and pi.branch_id = v_branch_id;
  end loop;
